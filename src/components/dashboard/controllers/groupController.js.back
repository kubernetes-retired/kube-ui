...
/* todo: replace */angular.module('k8s.app').controller('GroupCtrl', [
  '$scope',
  '$route',
  '$interval',
  '$stateParams',
  ...
  '$location',
  'lodash',
  function($scope, $route, $interval, $stateParams, k8sApi, $rootScope, $location, _) {
    'use strict';
    ...
    $scope.handleGroups = function(parts, selector) {
      ...
      var barrier = $scope.createBarrier(count, function() {
        ...
        $scope.selectedGroupBy = $stateParams.grouping;
      });
    ...
    var groups = $stateParams.grouping;
    if (!groups) {
      groups = '';
    }
    ...
    $scope.handleGroups(groups.split('/'), $stateParams.selector);
